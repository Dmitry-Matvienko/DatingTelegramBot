...
## Сейчас
- **Фаза**: Реализация **бесконечного циклического поиска анкет по категориям (Candidate Search Cycling)**:
  1. **Удаление ограничения на окончание анкет и 24-часовых лимитов**:
     - В сущность `User` добавлено поле `SearchCycleStartedAt` и создана миграция EF Core `Add_SearchCycleStartedAt_To_User`.
     - При исчерпании пула анкет текущего круга `MatchmakingService` автоматически перезапускает цикл поиска (`SearchCycleStartedAt = DateTime.UtcNow`), возвращая первого кандидата из начала очереди согласно 4-уровневому каскадному скорингу (ИИ -> Интересы -> Город -> Соседние города до 500 км).
  2. **Строгая изоляция категорий**:
     - Запросы кандидатов `BuildBaseCandidateQuery` гарантируют 100% непересекаемость категорий (`DatingTarget`: Общение/Отношения/18+), взаимных полов (`Gender`/`TargetGender`), возраста и стран/языков на всех кругах поиска.
  3. **Безопасное повторное оценивание**:
     - В `SearchService.RateCandidateAsync` и `ProfileRatingRepository.Update` поддержано обновление существующей оценки при повторном оценивании с корректным пересчетом среднего балла анкеты и сохранением взаимных симпатий/истории в БД.
  4. **Спецификации и тесты**:
     - Обновлены `docs/architecture/specs/matchmaking_scoring.md` и `search_and_ratings.md`, добавлен `ADR-017` в `decisions.md`.
     - Добавлены модульные тесты для автоматического циклирования, граничных условий и повторного оценивания (все 378 тестов решения: 100% green, 0 failures).
