...
## Сейчас
- **Фаза**: Реализация **динамических рандомизированных подсказок в анкетах пользователей при поиске (`Search Profile Tips`)**:
  1. **Пул и локализация подсказок**:
     - В `LocalizationService` добавлены 15 динамических подсказок (`Search_Tip_1` .. `Search_Tip_15`), переведенных на все 6 языков (RU, UK, EN, HI, PT, ID).
     - Подсказки охватывают правила связи при оценке 6+, контакты официального канала и поддержки (@TheBestDating, @KimeLowe65), предупреждения о мошенниках и спаме, советы по AI-описанию, дальности поиска, приветствиям, общим интересам и фото.
  2. **Отображение в карточках анкет**:
     - В `SearchPromptService` (`SendMatchCandidateCardAsync` и `SendCandidateCardAsync`) в конец текста карточки добавлен визуальный разделитель `——` и динамическая подсказка, случайно выбираемая в памяти через `Random.Shared`.
  3. **Zero-Overhead Архитектура**:
     - 0 обращений к БД, 0 дополнительных аллокаций, In-Memory $O(1)$ поиск через потокобезопасный `LocalizationService`.
  4. **Тестирование**:
     - Добавлены модульные тесты в `tests/DatingBot.UnitTests/Services/SearchTipsTests.cs` (все 405 тестов решения: 100% green, 0 failures, 0 warnings).
