...
## Сейчас
- **Фаза**: Реализация **выбора дальности поиска анкет (`SearchDistancePreference`) и обновление интерфейса фильтров**:
  1. **Выбор дальности при регистрации**:
     - Введен финальный шаг регистрации `UserState.Registration_SelectingSearchDistance` (после ввода AI-описания).
     - Добавлено сообщение с 4 кнопками выбора: «до 100 км», «до 500 км», «в пределах страны», «без ограничений».
     - При выборе значения анкета завершается (`IsCompleted = true`), пользователь переходит в `UserState.Active` и видит свою готовую анкету.
  2. **Редактирование дальности поиска в фильтрах профиля**:
     - В меню «👤 Мой профиль» -> «🔎 Фильтры» добавлена кнопка «📍 Дальность поиска» с выбором из 4 опций (с динамической отметкой `✅`).
     - Кнопка «Готово ✅» в меню фильтров расположена строго перед кнопкой «❌ Отмена».
  3. **Логика скоринга и репозитория**:
     - Опции «до 100 км» и «до 500 км» работают строго в пределах одной страны с отсечением по Haversine-расстоянию.
     - Опция «в пределах страны» отображает анкеты из любых городов страны пользователя.
     - Опция «без ограничений» снимает ограничение по стране (с сохранением языковой совместимости).
  4. **Тестирование и миграция**:
     - Создана миграция EF Core `Add_SearchDistance_To_UserProfile`.
     - Добавлены модульные тесты для регистрации, редактирования, фильтрации в matchmaking и UI клавиатур (все 391 тест решения: 100% green, 0 failures, 0 warnings).
