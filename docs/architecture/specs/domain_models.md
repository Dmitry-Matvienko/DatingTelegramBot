...
- `ReferredByUserId` (`Guid?`): Идентификатор пригласившего пользователя (если зарегистрирован по реферальной ссылке).
- `ReferralLink` (`ReferralLink?`): Собственная реферальная ссылка пользователя.
...
- `TopBoostUntil` (`DateTime?`): Дата и время окончания нахождения анкеты в топе поиска (по реферальной программе).
...
### 2.8. `ReferralLink`
Реферальная ссылка пользователя для привлечения новых участников.
- `Id` (`Guid`): Первичный ключ.
- `UserId` (`Guid`): Внешний ключ на создателя ссылки (`User`).
- `Code` (`string`): Уникальный реферальный код (`ref_XXXX`).
- `InvitedCount` (`int`): Количество привлеченных пользователей.
- `CreatedAt` (`DateTime`): Дата создания ссылки (UTC).
- `UpdatedAt` (`DateTime?`): Дата обновления (UTC).

### 2.9. `ReferralRecord`
Журнал переходов и регистраций по реферальным ссылкам.
- `Id` (`Guid`): Первичный ключ.
- `ReferralLinkId` (`Guid`): Внешний ключ на реферальную ссылку.
- `ReferrerUserId` (`Guid`): Внешний ключ на пригласившего пользователя.
- `ReferredUserId` (`Guid`): Внешний ключ на приглашенного нового пользователя (Unique).
- `CreatedAt` (`DateTime`): Дата и время перехода (UTC).
