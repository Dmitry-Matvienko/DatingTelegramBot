...
- Прикрепление интерактивной Inline-кнопки **«💬 Написать»** (`GetAdminProfileCardKeyboard`), открывающей чат с пользователем (`https://t.me/` или `tg://user?id=`), а также кнопок бана, удаления анкеты и перехода к следующей анкете.
...
### 3.7. Отчет по реферальной программе (Referral Leaderboard)
- В разделе «🎁 Реферальная программа» администраторам отображается дополнительная inline-кнопка **«📊 Отчет»** (`ref_admin_report`).
- По нажатию выводится топ-15 пользователей по числу привлеченных участников с кликабельными именами/ссылками на аккаунты Telegram и количеством приглашенных.
