...
3. **Автоисправление городов и геолокация**:
   - Если введенного пользователем города нет в базе данных, бот предлагает до 3 похожих городов (`SearchSuggestionsAsync`, алгоритм Левенштейна и префиксное сопоставление) через инлайн-кнопки.
   - Сразу после кнопок бот отправляет сообщение с инструкцией и выводит Reply-клавиатуру **«📍 Отправить геолокацию»** (`KeyboardButton.WithRequestLocation`).
   - При отправке геолокации (`message.Location`) бот обращается к `IGeocodingService` (Nominatim + BigDataCloud + локальный Fallback). Если город отсутствует в таблице `Cities`, он автоматически сохраняется в БД с координатами и привязывается к анкете пользователя.
