...
1. Если у пользователя есть непросмотренные входящие оценки \(\ge 6\) баллов, при переходе в поиск ему **в первую очередь** показываются карточки оценивших его людей (`SendRaterCardAsync`) с прикрепленной Inline-кнопкой **«💬 Написать»** (`GetRaterCardKeyboard`).
2. Оценив входящую карточку в ответ, пользователь либо получает взаимный мэтч (при оценке \(\ge 6\)), либо возвращается к общему пулу поиска.
