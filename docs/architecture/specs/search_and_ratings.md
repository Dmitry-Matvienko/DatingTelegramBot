...
- **Повторное оценивание при циклическом поиске**:
  ...
  \]
  - **Уведомление о недавней оценке (< 24 часов)**: Если анкета попадается повторно и оценивается в течение 24 часов с момента предыдущей оценки, бот отправляет оценивающему пользователю сервисное уведомление: `Notification_AlreadyRatedRecently` (*«ℹ️ Вы уже недавно оценивали этого пользователя.»*) на выбранном языке интерфейса.
