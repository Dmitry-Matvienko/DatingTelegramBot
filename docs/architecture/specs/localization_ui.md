...
1. **Инлайн-клавиатуры (`InlineKeyboardMarkup`)**:
   - Каждое действие анкеты (лайк, дизлайк, редактирование, жалобы) формируется фабриками в `src/DatingBot.Bot/Keyboards/`.
...
2. **Reply-клавиатуры (`ReplyKeyboardMarkup`)**:
   - Главное меню: `[🔍 Искать анкеты]`, `[👤 Мой профиль]`, `[🌐 Язык]`.
